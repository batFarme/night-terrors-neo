...
func _on_area_entered(hitbox: Hitbox2D):
	print('hit by ' + str(hitbox.owner) + '!')
	if hitbox.owner.has_method("iAmBullet"):
		print('i touhced my own bullet...')
		return
	elif hitbox.dmg != null:  #checks if collided area has a dmg value; if it doesn't, it either is a presence box or a hurtbox
